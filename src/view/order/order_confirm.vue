				...
				<div class="row">
					...
					<div class="col-xs-12 order-body">
						<div class="tiaokuan">
							...
							<div class="order-cf">
								<img src="../../../static/img/about/ddqr_web_03.jpg"/>
								<h5>无锡鸣珂里桃花源精品酒店</h5>
								<span>地址：无锡惠山区阳山镇麒麟湾度假村39号</span>
								<hr />
								<h4>退款政策：</h4>
								<p>
									变更：请至少提前4个工作日与我们联系变更，视酒店房态安排。
								</p>
								<p>
									退订：请至少提前4个工作日告知我们，否则不予退款，请谅解。
								</p>
								<p>
									节假日期间入住的订单不予退订。
								</p>
						...
						<div class="order-xinxi">
							<div class="yuding order-cf-xinxi">
								<h4>订单信息</h4>
								<ul>
									<li class="bianhao">
										订单编号：<span>x123408888</span>
									</li>
									<li class="type">
									房型：<span>{{data.type}}</span>
									</li>
									<li class="number">
										 间数：<span>{{data.js}}</span>
									</li>
									<li class="name">
									入住人：<span>{{data.rz_name}}</span>
									</li>
									<li class="time">
										<label>入住日期：<span>{{data.startTime}}</span><em>{{Time(data.startTime)}}</em></label>;
									    <label>离店日期：<span>{{data.endTime}}</span><em>{{Time(data.endTime)}}</em></label>;
									    <label>共<em>{{data.ws}}</em>晚</label>
									</li>
									<li class="tel">
									...
									<li class="zt">
										订单状态：<span>{{data.zt}}</span>
									</li>
								</ul>
					             <a href="#" class="qx">取消订单</a>	
					             <router-link to="/orderPay">我要支付</router-link>
								<hr />
							    <p>
							    	客服将于工作日2小时内予以处理，请在订单显示“未支付”状态后，予以支付！谢谢配合！
							    </p>
							</div>
						</div>
					</div>
...
<script>
	...
	export default{
		...
		created:function(){
			fetch('get','order/order_success','',(res)=>{
				console.log(res)
				this.data=res.data
